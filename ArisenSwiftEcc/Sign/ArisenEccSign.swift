...
/// ArisenEccSign manages ECC signing.
public class ArisenEccSign {

    static let k1SignMaxAttempts = 100

    /// Sign data with a K1 key for validation on an Arisen chain.
    ///
    /// - Parameters:
    ...
    public class func signWithK1(publicKey: Data, privateKey: Data, data: Data) throws -> Data { // swiftlint:disable:this function_body_length
        ...
        try privateKey.withUnsafeBytes { rawBufferPointer in
            ...
            guard let pkbytes = bufferPointer.baseAddress else {
                throw ArisenError(.keySigningError, reason: "Base address of privateKey is nil.")
            }
        ...
        for i in 1...k1SignMaxAttempts {
            ...
            try digest.withUnsafeBytes { rawBufferPointer in
                ...
                guard let digestBytes = bufferPointer.baseAddress else {
                    throw ArisenError(.keySigningError, reason: "Base address of digest is nil.")
                }
            ...
            do {
                ...
                signingError = ArisenError(.keySigningError, reason: error.ArisenError.localizedDescription)
                continue
            }
            let headerByte: UInt8 = 27 + 4 + UInt8(recid)

            // Check for canonical
            // ref https://github.com/Arisen/fc/blob/9a0ed1f85e38adc6ace8c944b5f5f725f4829ba2/src/crypto/elliptic_common.cpp#L161
        ...
        if let signature = signature {
            ...
            if let signingError = signingError {
                ...
                throw ArisenError(.keySigningError, reason: "Unable to create canonical signature after \(k1SignMaxAttempts) attempts")
            }
        }
    }
